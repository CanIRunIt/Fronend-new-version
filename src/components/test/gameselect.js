...
progressHandler() {
  ...
  render() {
   ...
   /* gamesjsonsort.map((game,title) => { */
    ...
    return (
      <div  key={title} style={{textAlign: 'center', marginTop: '3px', display: 'flex', justifyContent: 'space-around'}} className="gamedynamic">

{/*       <Button variant="contained" style={{background: 'linear-gradient(to right, #fdfc47, #24fe41)',textAlign: 'center', width: '60%', fontFamily: 'ZCOOL QingKe HuangYou'}} onClick={() => this.gamepickHandler(game.title, game.Intel_CPU, game.NVIDIA_Graphics, game.RAM)}>{game.title.replace(" system requirements","")}</Button> */}
      
      
      
      <button className={styles.btn} style={{}} onClick={() => this.gamepickHandler(game.title, game.Intel_CPU, game.NVIDIA_Graphics, game.RAM)}>
      <div class={styles.btn__content}>
      <span class="btn__content">{game.title.replace(" system requirements","")}_</span>
      </div>
      <span class={styles.btn__glitch}></span>
      <span class={styles.btn__label}>sys_req</span>
  ...
  return ( <div style={{ fontFamily: 'Tomorrow'}}>
    <h1 style={{ textAlign: 'center', marginTop:'1%'}} class={styles.topic}>choose a game to run the test_</h1>
    <div>      
    {games}
    </div>
    </div>
